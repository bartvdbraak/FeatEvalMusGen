...
do
  ffmpeg -i "$i" -ac $audio_channels -ab $audio_bitrate -ar $audio_sampling_rate "conv/$i.wav"
done

converted="conv/temp.wav"
sox conv/*.wav $converted
cd ../../
mkdir $dataset_path
...
echo "done"
rm -f "preprocess/$dataset_path/conv/$converted"